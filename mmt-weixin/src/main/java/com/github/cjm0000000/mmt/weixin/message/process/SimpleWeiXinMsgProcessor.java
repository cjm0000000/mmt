...
public final class SimpleWeiXinMsgProcessor extends AbstractPassiveMsgProcessor {
  ...
  public BaseMessage processLocationMsg(MmtConfig cfg,
    ...
    if (replyMsg == null) throw new WeiXinException("地理位置消息处理失败。");
    // save log
  ...
  public BaseMessage processVideoMsg(MmtConfig cfg, IVideo msg) {
    if (!(msg instanceof IVideo)) throw new WeiXinException("不是正确的视频格式。");
    return sendTextMessage((BaseMessage) msg, "亲，我暂时无法识别视频信息哦，您可以给我发文字消息。");
  }

  @Override
  public BaseMessage processVoiceMsg(MmtConfig cfg,
