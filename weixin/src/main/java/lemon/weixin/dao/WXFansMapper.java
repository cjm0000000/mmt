...
public interface WXFansMapper {
	 ...
	 * @return
	 */
	@Select("SELECT A.wxid,A.cust_id,A.nick_name,A.status,date_format(A.timestamp,'%Y-%m-%d %H:%i') timestamp FROM weixin_fans A WHERE A.cust_id=#{cust_id} AND A.wxid=#{wxid}")
	 ...
	 * @param fans
	 */
	@Update("UPDATE weixin_fans A SET A.nick_name=#{nick_name},A.status=#{status} WHERE A.wxid=#{wxid} AND A.cust_id=#{cust_id}")
